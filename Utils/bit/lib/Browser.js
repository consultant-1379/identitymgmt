define([
    ...
    var Browser = {
        ...
        waitForElementDisappeared: function(element, timeout) {
            return waitFor(function() {
                    return Browser.getElement(element) === null;
                },
                timeout,
                'Element ' + element + ' still present'
            );
        },
        ...
        waitForElementsWithValue: function(element, value, timeout) {
            return waitFor(function() {
                    var els = Browser.getElements(element);
                    var found = null;
                    for(var el in els) {
                        if (els[el] && els[el].getText() === value.toString()) {
                            found = els[el];
                            break;
                        }
                    }
        ...
        waitForIndexedElementWithValue: function(element, index, value, timeout) {
         ...
         waitForNotAvailableIndexedElementWithValue: function(element, index, value, timeout) {
            return waitFor(function() {
                    var el = Browser.getElements(element);
                    if (el[index] && el[index].getText() === value) {
                        return false;
                ...
                timeout,
                'Element ' + element + ' with value ' + value + ' found'
            );
        },
